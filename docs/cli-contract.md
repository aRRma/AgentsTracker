...
в папке текущего проекта (`ProjectCatalog.Normalize` с обеих сторон, сравнение с
разделителем), папка данных шлюза запрещена отдельно; расширение из белого списка в коде
...
`mcp__tg__approve` на каждую отправку: папку, тип и размер шлюз проверяет сам. Флаг есть в
CLI 2.1.261; что он действительно снимает карточку при `--permission-prompt-tool`, проверить
смоук-тестом после первого перезапуска (карточка на `tg:send_file` — значит не снял).
