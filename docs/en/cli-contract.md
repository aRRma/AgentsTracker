...
The gateway starts `agent acp` (`--model` before the subcommand; the key only as `CURSOR_API_KEY`
in the environment, never in argv). Then newline JSON-RPC: `initialize` (protocolVersion 1) →
