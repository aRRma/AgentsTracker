...
`cursor/generate_image` → `IOperatorConsole.SendFileAsync` (same host policy as Claude). Do not
attach the gateway MCP server in `session/new.mcpServers` — ACP cards would then also ask about
those tools. Pictures from the chat use the same inbox folder; `agent acp` gets it as `--add-dir`.
...
It is reset only on `SessionLost` from the CLI — details in `cli-contract.md`.
