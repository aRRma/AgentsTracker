...
С **поддельным** токеном проба живёт сколько нужно: `getUpdates` получает `Unauthorized`, в
логе повторяется «Telegram недоступен», а рабочий бот ничего не замечает — монитор, порт MCP и
лимиты при этом работают как обычно. С **настоящим** токеном проба живёт около минуты: бот
один, и `getUpdates` второму процессу отдаёт 409. MCP-конфиг конфликта не создаёт в любом
случае — он у каждого свой, `mcp-gateway-<pid>.json`.
...
`docs/en/*.md` — английский оригинал: его читает агент, на него ссылается `CLAUDE.md`. Соседние
`docs/*.md` — русское зеркало: из него собирается вики, на него ссылается `README.md` (всегда
русский). Правят оба файла одним коммитом; документ, оставшийся в одном языке, — ошибка.
`CLAUDE.md` только английский, `README.md` только русский.

Новый документ заводят сразу в двух языках и добавляют строку в `$pages` скрипта
`scripts\sync-wiki.ps1` — для русского файла: вики собирается из `docs/*.md`, `docs/en/` в неё
не попадает.
...
Имена страниц — латиницей по имени файла (`docs/monitor.md` → `monitor`), заголовок для
боковой панели берётся из первого `#` файла. Ссылки на файлы репозитория и картинки скрипт
переписывает в абсолютные: вики лежит в отдельном git-репозитории и относительных путей
к основному не видит. Новый файл в `docs/` попадёт в вики, только если добавить его
в таблицу `$pages` скрипта, — иначе он останется без страницы и без строки в панели.

Первую страницу GitHub заводит только через веб-интерфейс: пока её нет, репозитория
`<репозиторий>.wiki.git` не существует и клонирование падает с «Repository not found».
