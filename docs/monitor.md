...
в `rem`, тёмная тема через `color-scheme`, `prefers-reduced-motion` глушит анимацию,
в `forced-colors` у шкал и карточек появляются рамки. Одна тонкость: `role="status"` стоит
только на состоянии и соединении — на часах он превращается в спам для чтеца.
