...
у селектов, `caption`/`scope` у таблиц, график продублирован таблицей в `<details>`, размеры
в `rem`, тёмная тема через `color-scheme`, `prefers-reduced-motion` глушит анимацию лампы
и плавную прокрутку, в `forced-colors` у шкал и карточек появляются рамки.
