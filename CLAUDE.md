...
A bridge between Telegram and an agent (Claude Code or Cursor CLI), chosen by `Gateway:Agent`. A
single .NET 10 app on an always-on PC: a chat message → the agent process in the project folder →
"may I?" questions as buttons in Telegram → the agent's answer back into the chat. Developed on
Windows; runs on Linux in a container, there is no separate installer for macOS and Linux yet.
...
настройки». An agent key goes into `ClaudeOptions`/`Gateway:Claude` or `CursorOptions`/`Gateway:Cursor`,
a channel key into its
...
```
channel ─▶ ChatDispatcher ──▶ ChatWorker ──▶ IAgentBackend
                    ▲                         │ claude -p  or  agent acp
                    └── ApprovalBroker ◀── OperatorConsole
                           Claude: ClaudePermissionTool ◀── MCP http://127.0.0.1:<McpPort>/mcp
...
Cursor has no MCP permission tool: approvals travel on ACP stdin. A file into the chat is
`cursor/generate_image` → `IOperatorConsole.SendFileAsync` (same host policy as Claude). There is no
`--allowedTools` equivalent; do not add Claude's MCP server to `session/new.mcpServers` — Cursor
would then ask for those tools through ACP cards as well.
...
chosen yet). Cursor has no `--permission-mode`: `CursorBackend` maps `plan`/`ask`/`default` to ACP
`session/set_mode` (`plan`/`ask`/`agent`); `auto`/`dontAsk` still use `agent` but answer
`request_permission` without a card.
...
Claude session is issued by the gateway (`--session-id`), a Cursor session by ACP (`SessionStarted`).
It is reset only on `SessionLost` from the CLI — details in `docs/en/cli-contract.md`.
