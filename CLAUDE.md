...
Дымовой прогон после правок инфраструктуры: скрипт в scratchpad, `pwsh -File`, поддельный токен и
свои порты; через ~8 с проверить `/api/snapshot` (поле `agent`, `cliVersion`), `/api/limits`,
404 на `/` порта MCP и 401 на `POST /mcp` без токена — с заголовками
`Content-Type: application/json` и `Accept: application/json, text/event-stream`, иначе придёт 415.

Сборка падает с `MSB3021`, если шлюз запущен: exe заблокирован. Собирайте в другую папку —
`dotnet build src\AgentsTracker.Gateway -o <временная папка>` — именно проект, не решение:
`-o` на уровне `.slnx` даёт предупреждение NETSDK1194, к коду оно не относится.
...
Идущий запуск записан в `state.json` (`GatewayState.ActiveRun`: чат, превью промпта, сессия) —
`ChatWorker` ставит `BeginRun` перед запуском и `EndRun` в `finally`. Если при старте запись
на месте, прошлый экземпляр умер посреди работы (`Stop-Process` из другой сессии, падение):
`StartupNotice` (`Infrastructure/Telegram`, зовёт `TelegramBotService` после подключения)
шлёт «🔌 Шлюз запущен» всем из `AllowedUserIds`, а в чат прерванного запуска — ещё и «прерван,
напишите „продолжай“», пишет `run.end` с исходом `interrupted` и `RunRecord` для монитора.
Без этого перезапуск выглядел как молчание: сирота `claude.exe` дорабатывал невидимо,
а его ответ читать было некому. Пользователю, который ещё не писал боту, Telegram не даёт
...
- Правя русские тексты и windows-пути скриптом из Bash, помните про escape-последовательности:
  ...
  положите скрипт в scratchpad инструментом Write (не heredoc) и запускайте файлом; результат
  проверяйте `grep … | cat -v` и сборкой.
- Ревьюеру-сабагенту (`feature-dev:code-reviewer`) Bash недоступен: `git show`/`git diff` он
  не выполнит. Для сравнения «до/после» давайте ему пути к старым версиям файлов, выгруженным
  в scratchpad (`git show <коммит>:<путь> > …`), либо берите `general-purpose`.
