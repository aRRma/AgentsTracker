...
выбирает в два шага: сначала папка (`ME`, `MF`, `PILX`), потом репозиторий в ней страницами по 12.
Одна папка — промежуточный экран пропускается. Репозиториев бывает больше, чем влезает в
клавиатуру: без папок и страниц они были бы недостижимы.
