...
src/AgentsTracker.Agents.Cursor/         Cursor CLI behind the same contracts, ACP on stdin/stdout
...
`--permission-prompt-tool mcp__tg__approve`); Cursor asks on ACP stdin (`session/request_permission`).
The question becomes a card in the chat, the button becomes the answer. The diagram, the token file
and the timeouts — `docs/en/architecture.md`; the undocumented parts of the contract
(`updatedInput`, «Всегда», the truncated tail, ACP) — `docs/en/cli-contract.md`.
...
- Comments explain which failure the code prevents, not what the code does.
