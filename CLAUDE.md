...
CLI переопределяется конфигом без пересборки). Нажатие кнопки кладёт в очередь ровно слэш-команду без аргументов —
как если бы её набрали в чате; скилл с `argument-hint` показывается с подсказкой. Запуски
считаются в `state.json` (`SkillUsage`, пишут `SkillsScreen` и `ChatEnqueueTextHandler` для
набранных руками слэш-команд), и первой группой экран выносит «⭐ Частые» — до пяти самых
запускаемых; в группах источников порядок остаётся алфавитным.
