...
.DESCRIPTION
    BotToken и AllowedUserIds переехали из корня секции Gateway в Gateway:Channel:Settings:
    их читает только модуль канала. Старый конфиг новый шлюз не запускает — он говорит,
    что ключи переехали, и просит перенести их.
