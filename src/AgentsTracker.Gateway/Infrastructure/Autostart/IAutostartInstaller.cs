...
/// <param name="Description">Человеческое описание — его показывают в оснастке ОС.</param>
...
/// <summary>
/// Автозапуск средствами ОС. У каждой системы свой способ, но приём один: положить
/// файл-описание и позвать штатную утилиту — <c>schtasks</c>, <c>launchctl</c>,
/// <c>systemctl --user</c>. Поэтому интерфейс не знает ни про задачи Планировщика, ни про
/// unit-файлы, а реализации не тянут зависимостей и не мешают self-contained публикации.
/// </summary>
public interface IAutostartInstaller
{
    ...
    /// <summary>Регистрирует запись, перезаписывая одноимённую: обновление — это тот же install.</summary>
