...
/// <summary>
/// Правило, которое агент предлагает запомнить у себя по кнопке «Всегда» (у Claude —
/// <c>permission_suggestions</c> в <c>.claude/settings.local.json</c> проекта). Хост показывает
/// <see cref="Display"/> и возвращает <see cref="Raw"/> агенту как есть: форма правила — дело бэкенда.
/// </summary>
...
/// <summary>Запрос разрешения на инструмент. Вход — как прислал агент: карточка сама решает, что показать.</summary>
/// <param name="SuggestedRules">
/// Правила, которые агент готов записать у себя по «Всегда». Когда они есть, хост не
/// запоминает свою сигнатуру, а возвращает их в решении: правило агента обычно префиксное
...
/// Реализует хост (карточки с кнопками в Telegram, правила «всегда», аудит); зовут бэкенды
/// из своего канала подтверждений — у Claude это MCP-инструмент.
///
/// Таймаут и отмена не бросают: приходят отказом с текстом для агента. Так бэкенду не нужно
/// знать, как хост различает «не ответил» и «/stop», — он лишь передаёт причину агенту.
/// </summary>
